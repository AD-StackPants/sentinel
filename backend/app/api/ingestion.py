...
def trigger_live_weather_sync():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to dispatch Celery task: {str(e)}")
...
def get_task_status(task_id: str):
    ...
    try:
        ...
        if task_result.ready():
            result_data = task_result.result if not isinstance(task_result.result, Exception) else str(task_result.result)
